...
def test_wiki_entity_shows_type_pages_and_edges(wiki):
    out = mcp_server.wiki_entity("library:redis")
    ...
    assert mcp_server.wiki_entity("library:nonexistent") == "no such entity: library:nonexistent"


def test_wiki_neighbors_directions_and_filter(wiki):
    inc = mcp_server.wiki_neighbors("library:redis", direction="in")
    assert "project:atlas" in inc and "decision:auth-migration" in inc
    assert "pages:" in inc  # grounded

    # Outgoing from redis: none.
    assert mcp_server.wiki_neighbors("library:redis", direction="out") == "no neighbors for library:redis"
    # Predicate filter.
    filtered = mcp_server.wiki_neighbors("library:redis", predicate="uses", direction="in")
    assert "project:atlas" in filtered and "decision:auth-migration" not in filtered


def test_wiki_traverse_paths(wiki):
    out = mcp_server.wiki_traverse("project:atlas", depth=2)
    assert "project:atlas -> library:redis" in out


def test_wiki_impact_paths_and_grounding(wiki):
    out = mcp_server.wiki_impact("library:redis")
...
def test_wiki_graph_stats_counts_by_type(wiki):
    out = mcp_server.wiki_graph_stats()
...
def test_query_and_get_note_related_entities(wiki):
    q = mcp_server.wiki_query("redis")
    assert "related entities:" in q
    assert "library:redis" in q

    g = mcp_server.wiki_get("atlas")
