...
- **Graceful degradation.** Lose the graph cache → rebuild it from Markdown. Lose the state store → confidence (and thus edge confidence) falls back to its Markdown-only value.
...
**Graph backend (engine choice).** The graph engine sits behind a small `GraphBackend` interface, and the default is an **embedded SQLite** backend (an edges table with recursive-CTE traversal) — no separate server, and no dependency beyond the SQLite you already run for search and state. This is a deliberate future-proofing choice: because the graph is a rebuildable cache rather than a system of record, the engine carries no canonical data and is low-lock-in. When you reach Tier B, the same interface is implemented by Postgres + Apache AGE or Neo4j (selected by config); a graph-native server engine could be slotted in the same way, without touching ingest, search, or the query layer.

---

## Reusing the standard template & rules

Same six-part template — **CONTEXT / OBJECTIVE / BUILD / CONSTRAINTS / ACCEPTANCE / ON DONE** — and the same standing rules: everything runs offline with `MNESIS_LLM_STUB=1`; conventional commits; self-checking acceptance; keep `CLAUDE.md` in sync in the same commit. Prompts continue the global numbering (15 onward).
...
```
CONTEXT: Phases 1-2 are complete and green. Phase 3 adds a typed knowledge graph. This step records relations in Markdown (the canonical, durable representation) and validates them. No graph backend and no extraction change yet.
...
- Update CLAUDE.md:
    ...
    * Add the refined invariant: the graph is a rebuildable cache regenerated from Markdown; mnesis rebuild rebuilds search and graph; the durable state store is never cleared. Note that the graph engine sits behind a pluggable GraphBackend interface with an embedded SQLite default.
...
CONSTRAINTS:
- No graph backend, no graph queries, no ingest/extraction changes yet.
...
- MNESIS_LLM_STUB=1 must exercise the full path offline.
...
## Prompt 17 — Graph store: pluggable backend (SQLite default) + traversal

```
CONTEXT: Pages now carry entities and relations in frontmatter. Build the property graph as a rebuildable projection of that Markdown, behind a pluggable backend so the engine is never load-bearing. The default backend is embedded SQLite; the graph is a cache, so this is a low-lock-in choice.

OBJECTIVE: Implement src/mnesis/graph.py with a GraphBackend interface, a default SQLite backend, a backend factory, the rebuild-from-Markdown projection, and the core traversal primitives, integrated into mnesis rebuild.

BUILD:
- Define a GraphBackend abstract interface carrying the full surface the rest of the system uses, with NO engine-specific types in any signature:
    * clear(); add_entity(ref, type); add_edge(s, p, o, source_page, page_confidence); finalize() (dedup edges, compute aggregate confidence, mark demoted).
    * get_entity(ref) -> {type, pages, edges}.
    * neighbors(ref, predicate=None, direction="out").
    * traverse(ref, predicate=None, depth=2, include_demoted=False) -> reachable entities with the connecting edges and paths.
- Implement SqliteGraphBackend at mnesis/.index/graph.db (gitignored). Tables: entities(ref PK, type); edges(id, s, p, o, source_pages JSON, assertion_count, confidence, demoted). Dedup edges by (s, p, o), accumulating source_pages. confidence = noisy-OR of the asserting pages' Phase-2 confidence: 1 - prod(1 - conf_i). Traversal via recursive CTEs, depth-bounded and cycle-safe. Mark an edge demoted when every supporting page is stale/superseded; exclude demoted edges by default.
- Project page-level structural edges too: supersedes (page->page) and contradicts (page<->page) from frontmatter.
- Backend factory get_graph_backend() selected by config MNESIS_GRAPH_BACKEND (default "sqlite"). Document that Tier-B backends (Postgres+Apache AGE, Neo4j) or a server graph engine implement the same interface and are chosen here, with no changes elsewhere.
- rebuild_graph() -> summary: clear and repopulate the selected backend from store.list_pages(), then finalize(). Integrate into the `mnesis rebuild` path so the search index and graph rebuild together; neither clears the durable state store.

CONSTRAINTS:
- ALL graph access goes through the GraphBackend interface. No SQL or engine-specific calls may leak into ingest, search, mcp_server, or cli — the interface is the single integration point, so swapping engines is a config change, not a refactor.
- The graph holds nothing not derivable from Markdown (+ Phase-2 confidence). It is a pure cache.
- Recursive traversal must be depth-bounded and cycle-safe, and deterministic for a given corpus.
- Edges from stale/superseded pages are demoted, not deleted.

ACCEPTANCE:
- tests/test_graph.py: build from 3 fixture pages -> expected entities and typed edges exist with correct assertion_count and a confidence in (0,1); neighbors and depth-2 traverse return the expected entities and paths; an edge asserted by two pages outranks one asserted by one; making a supporting page stale demotes its edge (excluded by default); a cyclic relation set does not hang traversal; deleting mnesis/.index/ and rebuilding reproduces the graph identically.
- tests/test_graph_backend_contract.py: run the core assertions through the GraphBackend interface (not the concrete class) so a second backend could be dropped in and pass unchanged. `pytest -q` green.

ON DONE: run tests, commit ("feat(phase3): pluggable graph backend (sqlite default) and traversal"), report node/edge counts for the fixtures and confirm the interface is the only integration point.
```
...
- Reach the graph only through the GraphBackend primitives from Prompt 17.
...
- MCP tools (verify the SDK registration API against the installed mcp package):
    * mnesis_entity(ref) -> entity type, the pages that declare it, and its typed edges with confidences.
    * mnesis_neighbors(ref, predicate=None, direction="out") and mnesis_traverse(ref, predicate=None, depth=2).
    * mnesis_impact(entity, depth=3) -> affected entities with paths and grounding pages.
    * mnesis_graph_stats() -> node/edge counts by type, demoted-edge count.
  mnesis_query / mnesis_get results gain a "related entities" note.
...
- No new ranking logic here - reuse Prompt 18's primitives, which go through the GraphBackend interface.
...
ON DONE: run tests, commit ("feat(phase3): graph tools for MCP and CLI"), report the steps to call mnesis_impact from Claude Code.
```
...
- All reads/writes to the graph go through the GraphBackend interface (add interface methods if the lint needs them, rather than reaching into a concrete backend).
...
- scripts/demo_phase3.py (stub mode, no network), printing each step:
    ...
    2. mnesis rebuild -> build search index AND graph; print graph stats and the active backend name.
...
- tests/test_phase3_e2e.py: the programmatic regression asserting steps 1-4, plus: deleting mnesis/.index/ and running mnesis rebuild reproduces the graph, the search ranking, and confidences, while preserving the durable state store (access + review queue). Phase-1 and Phase-2 end-to-end tests still pass.
- Update README (verify-Phase-3 checklist; new commands: entity, neighbors, impact, graph-stats, graph-lint; note the MNESIS_GRAPH_BACKEND switch and its sqlite default), Makefile/justfile (rebuild already covers the graph; add graph-stats and graph-lint targets, plus demo-phase3), and confirm CLAUDE.md lists Phase 3 as in scope with Phases 4-6 deferred.

CONSTRAINTS:
- No network; demo and tests run with MNESIS_LLM_STUB=1.
...
8. Delete `mnesis/.index/` and `mnesis rebuild` — search, graph, and confidences all return; **access counts and the review queue survive**. Markdown remains the only source of truth.
...
- The invariant to enforce in review: **entities and relations are canonical (Markdown); the graph and all edge confidences are derived (cache).** If a diff writes a `confidence:` onto an edge in frontmatter, or treats the graph backend as a source of truth, that is the bug.
- The default graph backend is **embedded SQLite** — no external engine to install, version, or track against an upstream that might go away. Keep every graph access behind the `GraphBackend` interface so swapping in a Tier-B engine later (Postgres + Apache AGE, or Neo4j) stays a config change, not a rewrite. Because the graph is a rebuildable cache, that swap never risks canonical data.
