...
src/mnesis/               # the core (canonical store + tools)
  ...
  authz.py                # IAM4 the single PDP: RBAC role->permission matrix + scopes (role ∩ scope) + tenant match + within-tenant visibility; decide()/require_permission()/enforce (§16)
...
The web UI has **real interactive login** and per-request PDP authorization; the single **browser-injected bearer token is retired**. The HTTP app now has two distinct auth surfaces (`mcp_server.build_http_app`): **`/api/*`** = web **cookie sessions**; **`/mcp`** = agent **bearer credentials** (unchanged); **`/health`** = open.

- **Login/logout/reset.** `POST /api/auth/login` authenticates username/password via the IAM2 `LocalPasswordProvider` and issues an IAM3 web **session**, delivered as a **secure / httpOnly / SameSite** cookie (`mnesis_session`) — the raw session is never in the response body. `POST /api/auth/logout` revokes the session **immediately** (server-side) and clears cookies. `GET /api/auth/session` returns the resolved principal + effective permissions. `POST /api/auth/reset/request` (always `202`, no user enumeration) + `/api/auth/reset/confirm` drive the token-based reset; a completed reset revokes all the principal's sessions.
...
- **The permission model.** Fine-grained permissions over resources/actions: `pages:read`, `pages:write`, `pages:delete`, `graph:maintain`, `agents:run`, `users:manage`, `credentials:issue`, `egress:configure`, `tenants:manage`. Each belongs to a coarse **class** (`read`/`write`/`maintain`/`admin`) used for scope families and the coarse T4 actions. The **role → permission matrix** (`ROLE_PERMISSIONS`) is explicit: **`system_admin`** = all (but tenant-match bars it from any single tenant's data, so what it *does* is `tenants:manage`); **`admin`** (tenant-admin) = all page/graph/agent perms + `users:manage`/`credentials:issue`/`egress:configure` (not `tenants:manage`); **`member`** = `pages:read/write/delete` + `graph:maintain`; **`agent`** = member + `agents:run` (never admin/user/tenant/credential); **`readonly`** = `pages:read`.
